...
infix operator fun <T : YoutubeMusicInnertube.Item> YoutubeMusicInnertube.ItemsPage<T>?.plus(other: YoutubeMusicInnertube.ItemsPage<T>) =
    other.copy(
        items = (this?.items?.plus(other.items ?: emptyList()) ?: other.items)
            ?.distinctBy(YoutubeMusicInnertube.Item::key),
